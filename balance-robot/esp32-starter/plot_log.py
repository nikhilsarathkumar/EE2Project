...
SAVGOL_WINDOW_MS = 1000   # smoothing window in milliseconds — tune this
SAVGOL_POLY      = 2     # polynomial order (2 = quadratic)
...
def main():
    ...
    # Savitzky-Golay ideal smoothing: polynomial fit over a symmetric window
    # Zero phase (no lag), preserves peak shapes
    td_r_arr = np.array(td_r)
    dt_ms    = np.median(np.diff(t_ms)) if len(t_ms) > 1 else 10.0
    win      = max(int(SAVGOL_WINDOW_MS / dt_ms) | 1, SAVGOL_POLY + 2)  # must be odd and > poly order
    win      = win if win % 2 == 1 else win + 1
    td_ideal = np.full_like(td_r_arr, float('nan'))
    valid    = ~np.isnan(td_r_arr)
    if valid.sum() > win:
        td_ideal[valid] = savgol_filter(td_r_arr[valid], win, SAVGOL_POLY)
